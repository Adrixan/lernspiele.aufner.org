...
const ALL_CHALLENGE_IDS = allRooms.flatMap((room) => room.terminals.map((t) => t.challengeId));
/** Human-readable room names derived from room IDs */
function humanizeRoomId(id) {
    return id
        .replace(/^(floor\d+_|lobby_|server_|research_|surveillance_|archives_|comms_|executive_|manufacturing_|network_|ai_core_|rooftop_)/, '')
        .replace(/_/g, ' ')
...
export class TextAdventureEngine {
    ...
    enterRoom(roomId) {
        ...
        if (remaining.length > 0) {
            ...
            this.addNarrative('cipher', `I'm detecting ${String(count)} active terminal${count === 1 ? '' : 's'} in this area. Hack ${count === 1 ? 'it' : 'them all'} to proceed.`, 'CIPHER');
        }
        else {
            this.addNarrative('cipher', 'All terminals in this area are compromised. Move forward when ready.', 'CIPHER');
        }
        EventBus.emit({ type: 'ROOM_ENTER', payload: { roomId, firstVisit } });
    }
    getAvailableActions(completedChallenges) {
        ...
        for (const terminal of room.terminals) {
            if (mergedChallenges.includes(terminal.challengeId))
                continue;
            actions.push({
        ...
        for (const conn of room.connections) {
            ...
            if (!unlocked) {
                ...
                const remaining = (conn.requiredChallenges ?? []).filter((id) => !mergedChallenges.includes(id));
    ...
    executeAction(action, completedChallenges) {
        const room = this.getCurrentRoom();
        if (!room)
            return;
        ...
        switch (action.type) {
            case 'look': {
                ...
                if (remaining.length > 0) {
                    this.addNarrative('cipher', `${String(remaining.length)} terminal${remaining.length === 1 ? '' : 's'} still active in this area.`, 'CIPHER');
                }
                else {
    ...
    syncCompletedChallenges(challenges) {
    ...
    completeChallenge(challengeId) {
        ...
        if (remainingInRoom.length > 0) {
            this.addNarrative('cipher', `${String(remainingInRoom.length)} terminal${remainingInRoom.length === 1 ? '' : 's'} remaining in this area.`, 'CIPHER');
            return;
        }
        ...
        this.addNarrative('cipher', 'All terminals in this area are compromised. The path forward is open.', 'CIPHER');
        ...
        if (allFloorComplete) {
            ...
            if (intro) {
                ...
                this.addNarrative('system', `All challenges on floor ${String(room.floor)} complete! Access to floor ${String(nextFloor)} is now unlocked.`);
            }
            EventBus.emit({
                type: 'FLOOR_COMPLETE',
    ...
    isConnectionUnlocked(connection, completedChallenges) {
    ...
    static getGameVictory() {
        return gameVictory;
    }
    addNarrative(type, text, speaker) {
        const entry = {
...
//# sourceMappingURL=TextAdventureEngine.js.map