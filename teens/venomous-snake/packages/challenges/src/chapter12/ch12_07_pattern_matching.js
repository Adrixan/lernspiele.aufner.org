export const ch12_07_pattern_matching = {
    ...
    solutionCode: 'def classify_signal(signal):\n    if isinstance(signal, dict):\n        sig_type = signal.get("type")\n        if sig_type == "alert":\n            level = signal.get("level", 0)\n            if level >= 8:\n                return f"CRITICAL ALERT: Level {level}"\n            return f"Alert: Level {level}"\n        elif sig_type == "status":\n            msg = signal.get("message", "")\n            return f"Status: {msg}"\n    return "Unknown signal"\n\nprint(classify_signal({"type": "alert", "level": 9}))\nprint(classify_signal({"type": "alert", "level": 3}))\nprint(classify_signal({"type": "status", "message": "All clear"}))',
    testCases: [
        {
            id: 'tc01',
    ...
    hints: [
        { tier: 1, text: 'Use isinstance() to check types and dict.get() to safely access keys.' },
        { tier: 2, text: 'if/elif chains with dict lookups can handle structured data dispatch like match/case.' },
        { tier: 3, text: 'Solution: level >= 8 for critical, "status" for status check, "Unknown signal" as default' },
    ],
...
//# sourceMappingURL=ch12_07_pattern_matching.js.map