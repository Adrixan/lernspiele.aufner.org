...
const ERROR_INTROS = {
    SyntaxError: 'Ah, a SyntaxError. Python is very particular about grammar — more so than my last firmware update.',
    NameError: "NameError — you tried to use something that doesn't exist yet. Like my vacation days.",
    TypeError: "A TypeError. Python found a type mismatch, which is its polite way of saying 'that doesn't compute'.",
    IndentationError: 'IndentationError. Python has opinions about spacing. Strong, enforced, unforgiving opinions.',
    ValueError: 'ValueError. The type was right, but the value was... a creative choice.',
    IndexError: 'IndexError. You reached beyond the edge of the list. There be dragons — or rather, exceptions.',
    KeyError: "KeyError. That key doesn't exist in the dictionary. It's like knocking on a door that isn't there.",
    AttributeError: "AttributeError. That object doesn't have the property you're asking for. You can't get blood from a stone, or that method from that object.",
    ImportError: "ImportError. The module wasn't found. Either it's not installed or we've been spelling it wrong. Classic.",
    ZeroDivisionError: 'ZeroDivisionError. You divided by zero. Mathematicians have feelings about this.',
};
...
export class CipherAI {
    ...
    explainError(errorType, errorMessage) {
        this.mood = 'concerned';
        const intro = ERROR_INTROS[errorType] ??
    ...
    getReaction(passed, attempts, hintsUsed) {
        if (passed) {
...
//# sourceMappingURL=CipherAI.js.map