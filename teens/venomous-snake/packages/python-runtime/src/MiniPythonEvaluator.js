...
class PyError {
    ...
    line;
    obj;
    constructor(type, message, line, obj) {
        this.type = type;
...
// ─── Constructors ───────────────────────────────────────────────────────────
function mkInt(v) { return { kind: 'int', value: Math.trunc(v) }; }
function mkFloat(v) { return { kind: 'float', value: v }; }
function mkStr(v) { return { kind: 'str', value: v }; }
function mkBool(v) { return { kind: 'bool', value: v }; }
const NONE = { kind: 'none' };
function mkList(items) { return { kind: 'list', items }; }
function mkDict(entries) { return { kind: 'dict', entries }; }
function mkTuple(items) { return { kind: 'tuple', items }; }
function mkSet(items) {
    const unique = [];
    for (const item of items) {
        if (!unique.some(u => pyEqual(u, item)))
            unique.push(item);
    }
        ...
        if (callMethod) {
            try {
                ...
                if (result.kind === 'str')
                    return result.value;
                if (result.kind !== 'none')
                    return pyStr(result, callMethod);
            }
...
function pyEqual(a, b) {
    ...
    if (a.kind === 'bool' && (b.kind === 'int' || b.kind === 'float'))
        return (a.value ? 1 : 0) === b.value;
    if ((a.kind === 'int' || a.kind === 'float') && b.kind === 'bool')
...
function pyLt(a, b) {
    ...
    if ((a.kind === 'tuple' || a.kind === 'list') && (b.kind === 'tuple' || b.kind === 'list')) {
        ...
        for (let i = 0; i < len; i++) {
            if (pyEqual(a.items[i], b.items[i]))
                continue;
...
function pyContains(container, item) {
    ...
    if (container.kind === 'set')
        return container.items.some(i => pyEqual(i, item));
    if (container.kind === 'str' && item.kind === 'str')
        return container.value.includes(item.value);
    if (container.kind === 'dict')
        return container.entries.some(([k]) => pyEqual(k, item));
    return false;
}
function toNum(v) {
...
class ExprParser {
    ...
    peek() { return this.toks[this.pos] ?? { type: 'eof', val: '', pos: -1 }; }
    advance() { const t = this.peek(); this.pos++; return t; }
    expect(val) {
        const t = this.advance();
        if (t.val !== val)
            throw new PyError('SyntaxError', `expected '${val}', got '${t.val}'`);
        return t;
    }
    atEnd() { return this.peek().type === 'eof'; }
    match(val) {
        if (this.peek().val === val) {
            this.advance();
            return true;
        }
        return false;
    ...
    parseOr(env) {
        ...
        while (this.peek().val === 'or') {
            this.advance();
            const r = this.parseAnd(env);
            left = pyBool(left) ? left : r;
        }
        return left;
    }
    parseAnd(env) {
        let left = this.parseNot(env);
        while (this.peek().val === 'and') {
            this.advance();
            const r = this.parseNot(env);
            left = pyBool(left) ? r : left;
        }
        return left;
    }
    parseNot(env) {
        if (this.peek().val === 'not') {
            this.advance();
            return mkBool(!pyBool(this.parseNot(env)));
        }
    ...
    parseComparison(env) {
        ...
        while (ops.includes(this.peek().val) && this.peek().type !== 'str' && this.peek().type !== 'num' && this.peek().type !== 'fstr') {
            ...
            if (op === 'not') {
                ...
                left = mkBool(!pyContains(right, left));
                continue;
            }
            if (op === 'is') {
                if (this.peek().val === 'not') {
                    this.advance();
                    const right = this.parseAdd(env);
                    left = mkBool(!pyEqual(left, right));
                    continue;
                }
                const right = this.parseAdd(env);
                left = mkBool(left === right || pyEqual(left, right));
                continue;
            }
            if (op === 'in') {
                const right = this.parseAdd(env);
            ...
            if (op === '==')
                ...
                left = mkBool(pyLt(left, right));
            else if (op === '>')
                left = mkBool(pyLt(right, left));
            else if (op === '<=')
                left = mkBool(pyEqual(left, right) || pyLt(left, right));
            else if (op === '>=')
                left = mkBool(pyEqual(left, right) || pyLt(right, left));
        }
        // Handle 'not in'
        if (this.peek().val === 'not') {
            const save = this.pos;
    ...
    parseSlice(container, start, env) {
        this.expect(':');
        let stop = NONE;
        if (this.peek().val !== ']' && this.peek().val !== ':')
            stop = this.parseExpr(env);
        let step = NONE;
        if (this.match(':')) {
            if (this.peek().val !== ']')
                step = this.parseExpr(env);
        }
        this.expect(']');
        return this.applySlice(container, start, stop, step);
    }
    applySlice(container, startV, stopV, stepV) {
        ...
        if (container.kind === 'str')
            return mkStr(result.join(''));
        if (container.kind === 'tuple')
            return mkTuple(result);
        return mkList(result);
    }
    getIndex(container, idx) {
        if (container.kind === 'str') {
            const i = this.resolveIndex(container.value.length, idx);
            const ch = container.value[i];
            if (ch === undefined)
                throw new PyError('IndexError', 'string index out of range');
            return mkStr(ch);
        }
        if (container.kind === 'list' || container.kind === 'tuple') {
            ...
            if (item === undefined)
                throw new PyError('IndexError', `${container.kind} index out of range`);
            return item;
        }
        if (container.kind === 'dict') {
            const entry = container.entries.find(([k]) => pyEqual(k, idx));
            if (!entry)
                throw new PyError('KeyError', pyRepr(idx));
            return entry[1];
        }
        throw new PyError('TypeError', `'${pyTypeName(container)}' object is not subscriptable`);
    }
    resolveIndex(len, idx) {
        let i = toNum(idx);
        if (i < 0)
            i = len + i;
        return i;
    }
    getAttr(val, attr, env) {
        // String methods
        if (val.kind === 'str')
            return this.strMethod(val, attr);
        // List methods
        if (val.kind === 'list')
            return this.listMethod(val, attr);
        // Dict methods
        if (val.kind === 'dict')
            return this.dictMethod(val, attr);
        // Set methods
        if (val.kind === 'set')
            return this.setMethod(val, attr);
        // Object attributes
        if (val.kind === 'obj') {
    ...
    strMethod(s, name) {
        ...
        const builtinFn = (args, _kw) => {
            switch (name) {
                ...
                case 'replace': {
                    ...
                    if (count < 0)
                        return mkStr(sv.split(old).join(nw));
                    let result = sv;
                    let remaining = count;
                    while (remaining > 0) {
                ...
                case 'count': {
                    ...
                    let c = 0;
                    let idx = 0;
                    while ((idx = sv.indexOf(sub, idx)) !== -1) {
                        c++;
                        idx += sub.length;
                    }
                    return mkInt(c);
        ...
        return { kind: 'func', name, params: [], body: [], closure: new Env(), _native: builtinFn };
    }
    listMethod(list, name) {
        const builtinFn = (args) => {
            switch (name) {
                case 'append':
                    list.items.push(args[0] ?? NONE);
                    return NONE;
                case 'pop': {
                ...
                case 'extend': {
                    const other = args[0];
                    if (other?.kind === 'list' || other?.kind === 'tuple')
                        list.items.push(...other.items);
                    return NONE;
                }
                case 'sort': {
                    list.items.sort((a, b) => {
                ...
                case 'index': {
                    const idx = list.items.findIndex(i => pyEqual(i, args[0] ?? NONE));
                    if (idx === -1)
                        throw new PyError('ValueError', 'not in list');
                    return mkInt(idx);
                }
        ...
        const builtinFn = (args) => {
            switch (name) {
                ...
                case 'update': {
                    ...
                    if (other?.kind === 'dict') {
                        for (const [k, v] of other.entries) {
                            ...
                            if (existing >= 0)
                                dict.entries[existing] = [k, v];
                            else
                                dict.entries.push([k, v]);
                        }
                    }
                    return NONE;
                }
                case 'pop': {
                    ...
                    if (idx >= 0) {
                        const entry = dict.entries.splice(idx, 1)[0];
                ...
                case 'copy': return mkDict([...dict.entries.map(([k, v]) => [k, v])]);
                case 'clear': {
    ...
    setMethod(s, name) {
        const builtinFn = (args) => {
            switch (name) {
                case 'add': {
                    if (!s.items.some(i => pyEqual(i, args[0] ?? NONE)))
                        s.items.push(args[0] ?? NONE);
                    return NONE;
                }
                case 'remove': {
                    const idx = s.items.findIndex(i => pyEqual(i, args[0] ?? NONE));
                    if (idx === -1)
                        throw new PyError('KeyError', pyRepr(args[0] ?? NONE));
                ...
                case 'union': {
                    ...
                    if (other?.kind === 'set' || other?.kind === 'list') {
                        for (const item of other.items)
                            if (!result.some(i => pyEqual(i, item)))
                                result.push(item);
                    }
                    return mkSet(result);
                }
                case 'intersection': {
                    const other = args[0];
                    if (other?.kind === 'set' || other?.kind === 'list')
                        return mkSet(s.items.filter(i => other.items.some(j => pyEqual(i, j))));
                    return mkSet([]);
                }
                case 'difference': {
                    const other = args[0];
    ...
    applyBinOp(op, l, r) {
        ...
        if (op === '+' && l.kind === 'str' && r.kind === 'str')
            return mkStr(l.value + r.value);
        if (op === '*' && l.kind === 'str' && r.kind === 'int')
            return mkStr(l.value.repeat(Math.max(0, r.value)));
        if (op === '*' && l.kind === 'int' && r.kind === 'str')
            return mkStr(r.value.repeat(Math.max(0, l.value)));
        // List ops
        if (op === '+' && l.kind === 'list' && r.kind === 'list')
            return mkList([...l.items, ...r.items]);
        if (op === '*' && l.kind === 'list' && r.kind === 'int') {
            const res = [];
        ...
        // Tuple ops
        if (op === '+' && l.kind === 'tuple' && r.kind === 'tuple')
            return mkTuple([...l.items, ...r.items]);
        // Numeric ops
        const ln = toNum(l);
        const rn = toNum(r);
    ...
    parsePrimary(env) {
        ...
        if (tok.val === '{') {
            ...
            if (this.peek().val === ':') {
                ...
                if (this.peek().val === 'for') {
                    return this.parseDictComp(first, val, env);
                }
                const entries = [[first, val]];
                while (this.match(',')) {
                    if (this.peek().val === '}')
                        break;
        ...
        if (tok.val === 'lambda') {
            ...
            const bodyToks = [...this.toks.slice(bodyStart, this.pos), { type: 'eof', val: '', pos: -1 }];
        ...
        if (tok.type === 'id' && !KEYWORDS.has(tok.val)) {
            ...
            if (val === undefined)
                throw new PyError('NameError', `name '${tok.val}' is not defined`);
            return val;
        }
        throw new PyError('SyntaxError', `unexpected token: '${tok.val}'`);
    }
    parseLambdaParams(_env) {
        const params = [];
    ...
    parseGeneratorExpr(env) {
        ...
        const exprToks = [...this.toks.slice(exprStart, this.pos), { type: 'eof', val: '', pos: -1 }];
        this.expect('for');
        ...
        if (this.peek().val === 'if') {
            ...
            condToks = [...this.toks.slice(condStart, this.pos), { type: 'eof', val: '', pos: -1 }];
        }
        ...
        for (const item of iterItems) {
            ...
            if (condToks) {
                ...
                if (!pyBool(p.evalTokens(condToks, loopEnv)))
                    continue;
            }
            const p = new ExprParser(this.executor);
    ...
    parseListCompFull(env) {
        ...
        while (this.peek().val === 'for' || this.peek().val === 'if') {
            if (this.peek().val === 'for') {
                ...
                while (!this.atEnd()) {
                    ...
                    else if ((t.val === 'for' || t.val === 'if') && id === 0)
                        break;
                    this.advance();
                }
                const iterToks = [...this.toks.slice(iterStart, this.pos), { type: 'eof', val: '', pos: -1 }];
                clauses.push({
                    kind: 'for', varNames, iterToks,
                    iterExpr: (e) => { const p = new ExprParser(this.executor); return p.evalTokens(iterToks, e); },
                });
            }
            else {
                ...
                while (!this.atEnd()) {
                    ...
                    else if ((t.val === 'for' || t.val === 'if') && cd === 0)
                        break;
                    this.advance();
                }
                const condToks = [...this.toks.slice(condStart, this.pos), { type: 'eof', val: '', pos: -1 }];
        ...
        const items = [];
        const execClauses = (idx, currentEnv) => {
    ...
    parseDictCompFull(env) {
        ...
        while (!this.atEnd()) {
            ...
            if (t.val === '(' || t.val === '[' || t.val === '{')
                ...
                depth--;
            else if (t.val === ':' && depth === 0 && colonPos < 0)
                colonPos = this.pos;
            else if (t.val === 'for' && depth === 0)
                break;
            this.advance();
        ...
        if (colonPos >= 0) {
            ...
            const keyToks = [...this.toks.slice(exprStart, colonPos), { type: 'eof', val: '', pos: -1 }];
            const valToks = [...this.toks.slice(colonPos + 1, this.pos), { type: 'eof', val: '', pos: -1 }];
            this.expect('for');
            const varNames = this.parseTargetNames();
            this.expect('in');
            const iter = this.parseOr(env);
    ...
    parseDictComp(_keyExpr, _valExpr, env) {
        this.expect('for');
        const varNames = this.parseTargetNames();
        this.expect('in');
        const iter = this.parseOr(env);
    ...
    getIterable(v) {
        if (v.kind === 'list' || v.kind === 'tuple' || v.kind === 'set')
            return v.items;
        if (v.kind === 'str')
            return v.value.split('').map(mkStr);
        if (v.kind === 'dict')
            return v.entries.map(([k]) => k);
        if (v.kind === 'generator') {
            if (v.items)
                return v.items;
            // Consume iterator
    ...
    applyFormat(val, spec) {
        ...
        if (width > formatted.length) {
            ...
            if (align === '<')
                formatted = formatted + fill.repeat(pad);
            else if (align === '^') {
                const l = Math.floor(pad / 2);
                formatted = fill.repeat(l) + formatted + fill.repeat(pad - l);
            }
            else if (align === '=' || fill === '0') {
                // Numeric padding: insert after sign
                const isNeg = formatted.startsWith('-');
                if (isNeg)
                    formatted = '-' + fill.repeat(pad) + formatted.substring(1);
                else
                    formatted = fill.repeat(pad) + formatted;
            }
            else
                formatted = fill.repeat(pad) + formatted;
        }
...
class Executor {
    ...
    queueInput(val) { this.inputQueue.push(val); }
    execute(code) {
        this.stdout = [];
        this.env = new Env();
        this.stepCount = 0;
    ...
    parseLines(code) {
        ...
        for (let i = 0; i < rawLines.length; i++) {
            ...
            if (trimmed === '' || trimmed.startsWith('#')) {
                if (bracketDepth > 0 && pending !== '')
                    continue; // skip blank/comment lines inside open brackets
                continue;
            }
    ...
    executeBlock(lines, start, end, env) {
        let i = start;
        while (i < end) {
            this.stepCount++;
            if (this.stepCount > this.maxSteps)
                throw new PyError('RuntimeError', 'maximum execution steps exceeded');
            const lineResult = this.executeLine(lines, i, end, env);
            if (lineResult.signal)
                return lineResult.signal;
            i = lineResult.nextLine;
        }
        return undefined;
    }
    collectYields(lines, start, end, env, yields) {
        let i = start;
        while (i < end) {
            this.stepCount++;
            if (this.stepCount > this.maxSteps)
                        ...
                        if (varNames.length === 1) {
                            ...
                            if (item.kind === 'tuple' || item.kind === 'list') {
                                for (let k = 0; k < varNames.length; k++)
                                    env.set(varNames[k], item.items[k] ?? NONE);
                            }
                        }
            ...
            const lineResult = this.executeLine(lines, i, end, env);
            if (lineResult.signal instanceof ReturnSignal)
                return;
            if (lineResult.signal instanceof BreakSignal)
                return;
            i = lineResult.nextLine;
        }
    }
    executeLine(lines, lineIdx, end, env) {
        ...
        // break
        if (text === 'break')
            return { nextLine: lineIdx + 1, signal: new BreakSignal() };
        // continue
        if (text === 'continue')
            return { nextLine: lineIdx + 1, signal: new ContinueSignal() };
        // return
        if (text === 'return' || text.startsWith('return ')) {
            const expr = text === 'return' ? '' : text.substring(7).trim();
            if (!expr)
                return { nextLine: lineIdx + 1, signal: new ReturnSignal(NONE) };
        ...
        if (text.startsWith('yield ')) {
            const expr = text.substring(6).trim();
            const val = this.evalExpr(expr, env);
            return { nextLine: lineIdx + 1, signal: new YieldSignal(val) };
        }
        if (text === 'yield') {
            return { nextLine: lineIdx + 1, signal: new YieldSignal(NONE) };
        }
        // assert
        ...
        if (text.startsWith('raise ')) {
            ...
            if (val.kind === 'obj' && val._exType) {
                throw new PyError(val._exType, val._exMessage ?? '', undefined, val);
            }
            if (val.kind === 'obj')
                throw new PyError(val.cls.name, pyStr(val.attrs.get('message') ?? val.attrs.get('args') ?? mkStr('')), undefined, val);
            if (val.kind === 'str')
                throw new PyError('Exception', val.value);
            throw new PyError('Exception', pyStr(val));
        }
        if (text === 'raise')
            throw new PyError('Exception', '');
        // Decorator
        if (text.startsWith('@')) {
    ...
    findBlockEnd(lines, start, end) {
        if (start >= end)
            return start;
        const baseIndent = lines[start].indent;
        let i = start + 1;
        while (i < end && lines[i].indent > baseIndent)
            i++;
        return i;
    }
    executeIf(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
    ...
    executeFor(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        const text = line.text;
        ...
        for (const item of items) {
            ...
            if (signal instanceof ReturnSignal)
                return { nextLine: elseEnd, signal };
        }
    ...
    executeWhile(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        ...
        while (pyBool(this.evalExpr(condStr, env))) {
            ...
            if (iterations > this.maxSteps)
                throw new PyError('RuntimeError', 'infinite loop detected');
            const signal = this.executeBlock(lines, bodyStart, bodyEnd, env);
            if (signal instanceof BreakSignal)
                break;
            if (signal instanceof ContinueSignal)
                continue;
            if (signal instanceof ReturnSignal)
                return { nextLine: bodyEnd, signal };
        }
        return { nextLine: bodyEnd };
    }
    executeDef(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        const text = line.text;
        ...
        const func = { kind: 'func', name, params, body: bodyLines, closure: env };
        // Check if body contains yield — mark as generator
        if (bodyLines.some(l => /\byield\b/.test(l.text))) {
            func._isGenerator = true;
        }
        env.set(name, func);
        return { nextLine: bodyEnd };
    }
    parseParams(paramsStr, env) {
        if (!paramsStr.trim())
            return [];
        const params = [];
        ...
        for (const part of parts) {
            ...
            if (nameStr.startsWith('**')) {
                ...
                if (colonIdx >= 0)
                    nameStr = nameStr.substring(0, colonIdx).trim();
    ...
    executeClass(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        ...
        const cls = { kind: 'class', name, ...(parentName !== undefined ? { parentName } : {}), body: bodyLines, env: classEnv, methods, attrs };
        env.set(name, cls);
        return { nextLine: bodyEnd };
    }
    executeTry(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        const baseIndent = line.indent;
        ...
        try {
            ...
            if (e instanceof PyError) {
                        ...
                        if (clause.varName) {
                            if (e.obj) {
                                ...
                                const errCls = registeredCls?.kind === 'class' ? registeredCls
                                    : { kind: 'class', name: e.type, body: [], env: new Env(), methods: new Map(), attrs: new Map() };
                                const errObj = {
                                    kind: 'obj',
                                    cls: errCls,
                ...
                if (!caught) {
                    ...
                    if (finallyClause)
                        this.executeBlock(lines, finallyClause.start, finallyClause.end, env);
                    throw e;
                }
            }
            else
        ...
        if (finallyClause) {
            ...
            if (finSignal instanceof ReturnSignal)
                signal = finSignal;
        }
        return { nextLine: i, ...(signal !== undefined ? { signal } : {}) };
    }
    executeWith(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        const bodyStart = lineIdx + 1;
        const bodyEnd = this.findBlockEnd(lines, lineIdx, end);
    ...
    executeMatch(lines, lineIdx, end, env) {
        const line = lines[lineIdx];
        const baseIndent = line.indent;
        ...
        while (i < end) {
            ...
            if (caseLine.indent <= baseIndent)
                break;
            if (!caseLine.text.startsWith('case ')) {
                i++;
                continue;
            }
            ...
            if (matched) {
                if (guard) {
                    if (!pyBool(this.evalExpr(guard, env))) {
                        i = caseBodyEnd;
                        continue;
                    }
                }
    ...
    executeAssignmentOrExpr(lines, lineIdx, env) {
        ...
        if (target.includes(',')) {
            ...
            if (val.kind === 'tuple' || val.kind === 'list') {
                for (let i = 0; i < names.length; i++) {
                    const n = names[i];
                    if (n)
                        this.assignToTarget(n, val.items[i] ?? NONE, env);
                }
            }
            return;
        }
    }
    findAssignOp(text, op) {
        let depth = 0;
        let inStr = false;
        let strCh = '';
        for (let i = 0; i < text.length; i++) {
    ...
    makeCallMethod(env) {
        ...
        return (obj, methodName) => {
            let method = obj.cls.methods.get(methodName) ?? obj.attrs.get(methodName);
            // Walk parent chain
            ...
            if (!method || method.kind !== 'func')
                throw new PyError('AttributeError', `'${obj.cls.name}' has no '${methodName}'`);
            // Bind self via closure like bindMethod does
            const bound = { ...method, closure: (() => { const e = new Env(method.closure); e.set('__self__', obj); return e; })() };
            return self.callValue(bound, [], new Map(), env);
        };
    }
    callValue(callee, args, kwargs, _env) {
        // Native function (from method wrappers)
        const native = callee?._native;
        if (typeof native === 'function') {
            return native(args, kwargs);
        }
        // Built-in function
        if (callee.kind === 'func' && callee._builtin) {
            const boundSelf = callee.closure?.get?.('__self__');
            return callee._builtin(args, kwargs, _env, boundSelf);
        }
        // User-defined function
        if (callee.kind === 'func') {
            const func = callee;
            // Generator function — return a generator object with pre-collected values
            if (func._isGenerator) {
                ...
                const gen = { kind: 'generator', items: yieldedValues, index: 0 };
                return gen;
            }
            ...
            if (func._lambdaToks) {
                ...
                return p.evalTokens(func._lambdaToks, funcEnv);
            }
        ...
        if (callee.kind === 'class') {
            const cls = callee;
            const obj = { kind: 'obj', cls, attrs: new Map(cls.attrs) };
    ...
    isSubclassOf(childType, parentType, env) {
        const childCls = env.get(childType) ?? this.env.get(childType);
        if (!childCls || childCls.kind !== 'class')
            return false;
        ...
        while (current) {
            ...
            if (!parentCls || parentCls.kind !== 'class')
                return false;
            current = parentCls.parentName;
        }
        return false;
    }
    handleImport(text, env) {
    ...
    getModule(name) {
        if (name === 'json')
            return this.createJsonModule();
        if (name === 're')
            return this.createReModule();
        if (name === 'math')
            return this.createMathModule();
        if (name === 'random')
            return this.createRandomModule();
        if (name === 'io')
            return this.createIoModule();
        if (name === 'os')
            return this.createOsModule();
        if (name === 'string')
            return this.createStringModule();
    ...
    createJsonModule() {
        ...
        const dumps = {
            ...
            _builtin: (args) => mkStr(JSON.stringify(this.valToJs(args[0] ?? NONE))),
        };
    ...
    createReModule() {
        ...
        const sub = {
            ...
            _builtin: (args) => {
                ...
                try {
                    return mkStr(str.replace(new RegExp(pat, 'g'), repl));
                }
                catch {
                    return mkStr(str);
                }
    ...
    createMathModule() {
        ...
        const mathFn = (name, fn, returnInt = false) => ({
        ...
        attrs.set('pow', {
            ...
            _builtin: (args) => mkFloat(Math.pow(toNum(args[0] ?? mkInt(0)), toNum(args[1] ?? mkInt(0)))),
        });
    ...
    jsToVal(v) {
        ...
        if (typeof v === 'boolean')
            return mkBool(v);
        if (Array.isArray(v))
            return mkList(v.map(i => this.jsToVal(i)));
        if (typeof v === 'object') {
            const entries = Object.entries(v).map(([k, val]) => [mkStr(k), this.jsToVal(val)]);
            return mkDict(entries);
        }
    ...
    valToJs(v) {
        switch (v.kind) {
            ...
            case 'list':
            case 'tuple': return v.items.map(i => this.valToJs(i));
            case 'dict': {
                const o = {};
                for (const [k, val] of v.entries)
                    o[pyStr(k)] = this.valToJs(val);
                return o;
            }
    ...
    registerBuiltins() {
        ...
        builtin('isinstance', (args) => {
            ...
            if (cls.kind === 'class' && obj.kind === 'obj') {
                if (obj.cls.name === cls.name)
                    return mkBool(true);
                // Check parent chain
                let current = obj.cls;
                while (current?.parentName) {
                    if (current.parentName === cls.name)
                        return mkBool(true);
            ...
            const typeMap = { dict: 'dict', list: 'list', str: 'str', int: 'int', float: 'float', bool: 'bool', tuple: 'tuple', set: 'set' };
            const clsName = cls.kind === 'class' ? cls.name : cls.kind === 'func' ? cls.name : '';
            const expected = typeMap[clsName];
            if (expected)
                return mkBool(pyTypeName(obj) === expected);
            return mkBool(false);
        });
        builtin('hasattr', (args) => {
            ...
            if (obj.kind === 'obj')
                return mkBool(obj.attrs.has(name) || obj.cls.methods.has(name));
            return mkBool(false);
        });
        ...
        builtin('len', (args) => {
            const v = args[0] ?? NONE;
            if (v.kind === 'str')
                return mkInt(v.value.length);
            if (v.kind === 'list' || v.kind === 'tuple' || v.kind === 'set')
                return mkInt(v.items.length);
            if (v.kind === 'dict')
                return mkInt(v.entries.length);
        ...
        builtin('int', (args) => {
            ...
            if (v.kind === 'int')
                return v;
            if (v.kind === 'float')
                return mkInt(Math.trunc(v.value));
            if (v.kind === 'bool')
                return mkInt(v.value ? 1 : 0);
            if (v.kind === 'str') {
                const base = args[1]?.kind === 'int' ? args[1].value : 10;
                const n = parseInt(v.value.trim(), base);
                if (isNaN(n))
                    throw new PyError('ValueError', `invalid literal for int() with base ${base}: '${v.value}'`);
                return mkInt(n);
            }
        ...
        builtin('float', (args) => {
            ...
            if (v.kind === 'int')
                return mkFloat(v.value);
            if (v.kind === 'bool')
                return mkFloat(v.value ? 1.0 : 0.0);
            if (v.kind === 'str') {
                const n = parseFloat(v.value.trim());
                if (isNaN(n) && v.value.trim() !== 'nan')
                    throw new PyError('ValueError', `could not convert string to float: '${v.value}'`);
                return mkFloat(n);
            }
        ...
        builtin('abs', (args) => {
            const v = args[0] ?? mkInt(0);
            if (v.kind === 'int')
                return mkInt(Math.abs(v.value));
            if (v.kind === 'float')
                return mkFloat(Math.abs(v.value));
        ...
        builtin('min', (args) => {
            ...
            if (items.length === 0)
                throw new PyError('ValueError', 'min() arg is an empty sequence');
            return items.reduce((a, b) => pyLt(a, b) ? a : b);
        });
        builtin('max', (args) => {
            const items = args.length === 1 && (args[0]?.kind === 'list' || args[0]?.kind === 'tuple') ? args[0].items : args;
            if (items.length === 0)
                throw new PyError('ValueError', 'max() arg is an empty sequence');
        ...
        builtin('sum', (args) => {
            ...
            return iterable.reduce((acc, item) => {
                return self.parser.applyBinOp('+', acc, item);
            }, start);
        });
        builtin('range', (args) => {
            ...
            if (step === 0)
                throw new PyError('ValueError', 'range() arg 3 must not be zero');
            const items = [];
            if (step > 0) {
                for (let i = start; i < stop; i += step)
                    items.push(mkInt(i));
        ...
        builtin('dict', (args) => {
            ...
            const v = args[0];
            if (v.kind === 'dict')
                return mkDict([...v.entries]);
            if (v.kind === 'list') {
                const entries = v.items.map(item => {
                    if (item.kind === 'tuple' || item.kind === 'list')
                        return [item.items[0] ?? NONE, item.items[1] ?? NONE];
                    return [item, NONE];
                });
                return mkDict(entries);
            }
        ...
        builtin('filter', (args) => {
            ...
            const items = self.parser.getIterable(iter);
            if (!fn || fn.kind === 'none')
                return mkList(items.filter(item => pyBool(item)));
        ...
        builtin('super', () => {
            ...
            if (selfObj?.kind === 'obj' && selfObj.cls.parentName) {
                ...
                if (parentCls?.kind === 'class') {
                    ...
                    const superObj = { kind: 'obj', cls: parentCls, attrs: selfObj.attrs };
                    return superObj;
                }
            }
            return NONE;
        ...
        for (const exName of exceptionTypes) {
            ...
            strFunc._builtin = (_args, _kw, _callEnv, boundSelf) => {
            ...
            const methods = new Map([['__init__', initFunc], ['__str__', strFunc]]);
            ...
            realInit._builtin = (args, _kw, _callEnv, boundSelf) => {
                // Find self from boundSelf, envStack, or fallback
                let selfObj = boundSelf;
...
//# sourceMappingURL=MiniPythonEvaluator.js.map