...
export class XPSystem {
    ...
    getLevelForXP(xp) {
        ...
        if (result === undefined) {
            ...
            if (first === undefined)
                throw new Error('LEVEL_THRESHOLDS must not be empty');
            return first;
        }
        return result;
    }
     ...
     */
    calculateXPReward(baseXP, attempts, hintsUsed, timeMs) {
        let multiplier = 1.0;
        if (attempts === 1)
            multiplier += BONUS_FIRST_TRY;
        if (hintsUsed === 0)
            multiplier += BONUS_NO_HINTS;
        if (timeMs < 60_000)
            multiplier += BONUS_SPEED_FAST;
        else if (timeMs < 120_000)
            multiplier += BONUS_SPEED_OK;
