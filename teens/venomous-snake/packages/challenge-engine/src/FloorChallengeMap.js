 ...
 */
export function getNextUnlockedFloor(completedChallenges) {
    let maxUnlocked = 0;
...
export function getFloorProgress(floorNumber, completedChallenges) {
    const config = getFloorConfig(floorNumber);
    if (config === undefined)
...
export function getFloorNumberFromId(floorId) {
    if (floorId === 'lobby')
        return 0;
...
export function getFloorIdFromNumber(floorNumber) {
    return floorNumber === 0 ? 'lobby' : `floor_${floorNumber}`;
}
//# sourceMappingURL=FloorChallengeMap.js.map