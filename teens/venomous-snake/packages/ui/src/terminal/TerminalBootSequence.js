 ...
 */
export function TerminalBootSequence({ onComplete, lineDelay = 260, forceReplay = false, }) {
    ...
    if (_bootShown && !forceReplay && visibleCount === 0)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: BOOT_STYLES }), _jsxs("div", { className: "boot-seq-root", role: "status", "aria-label": "Terminal boot sequence", children: [_jsx("div", { className: "boot-seq-scanlines", "aria-hidden": "true" }), _jsx("div", { className: "boot-seq-lines", children: BOOT_LINES.slice(0, visibleCount).map((line, i) => (_jsx("p", { className: "boot-seq-line", children: line === '> _' ? (_jsxs(_Fragment, { children: ['> ', _jsx("span", { className: "boot-seq-cursor" })] })) : (line) }, i))) })] })] }));
}
const BOOT_STYLES = `
.boot-seq-root {
  position: absolute;
...
`;
//# sourceMappingURL=TerminalBootSequence.js.map