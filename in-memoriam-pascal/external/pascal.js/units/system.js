...
function SYSTEM(st) {
  function init() {
    ...
    ir.push(['declare i32 @_pascal_read_string(i8*, i32)']);  // Custom string READ function
    ir.push(['declare double @drand48()']);
    ir.push(['declare i32 @lrand48()']);
    ir.push(['declare void @exit(i32) noreturn nounwind']);
    ir.push(['declare void @memset(i8*, i8, i64)']);
    ir.push(['']);
  ...
  // I/O routines
  //
  // Custom string READ function that uses input queue instead of blocking scanf
  // This is called by the generated IR for STRING READ operations
  function _pascal_read_string(bufferPtr, maxLen) {
    // This function will be injected via Module.addFunction at runtime
    // It reads from the Pascal input queue
  ...
  function READ(ast, cparams) {
    ...
    // Check if we're reading a string - if so, use custom function instead of scanf
    var hasStringParam = cparams.some(function (p) { return p.type && p.type.name === 'STRING'; });

    if (hasStringParam) {
      // For string parameters, generate call to custom _pascal_read_string function
      for (var i = 0; i < cparams.length; i++) {
        var param = cparams[i];
        if (param.type && param.type.name === 'STRING') {
          var pre = st.new_name('READ_STRING');
          var buf = '%' + pre + 'buf';
          var bufsize = 256;
          var bufPtr = '%' + pre + 'bufptr';
          var call = '%' + pre + 'call';

          // Allocate buffer
          ir.push('  ' + buf + ' = alloca [' + bufsize + ' x i8]');
          ir.push('  ' + bufPtr + ' = getelementptr inbounds [' + bufsize + ' x i8]* ' + buf + ', i32 0, i32 0');

          // Initialize buffer to zeros
          ir.push('  call void @memset(i8* ' + bufPtr + ', i8 0, i64 ' + bufsize + ')');

          // Call custom read string function instead of scanf
          ir.push('  ' + call + ' = call i32 @_pascal_read_string(i8* ' + bufPtr + ', i32 ' + bufsize + ')');

          // Store buffer pointer to parameter
          ir.push('  store i8* ' + bufPtr + ', i8** ' + param.istack);

          ir.push('  ; STRING READ using custom function');
        }
      }
      ir.push('  ; READ finish');
      return ir;
    ...
    // Original scanf-based logic for non-string types
    for (var i = 0; i < cparams.length; i++) {
      var param = cparams[i],
      ...
      // STRING requires special handling because scanf with stdin can block in browser.
      // We use stack-allocated buffer to avoid malloc issues.
      // Buffer is initialized with memset before scanf reads into it.
      if (param.type.name === 'STRING') {
        // Allocate buffer for the string on stack (256 bytes is standard for Pascal strings)
        var buf = '%' + pre + 'buf';
        var bufsize = 256;

        ir.push('  ; STRING READ - stack-allocated buffer with initialization');

        // Allocate stack buffer
        ir.push('  ' + buf + ' = alloca [' + bufsize + ' x i8]');
        var buf_ptr = '%' + pre + 'buf_ptr';
        ir.push('  ' + buf_ptr + ' = getelementptr inbounds [' + bufsize + ' x i8]* ' + buf + ', i32 0, i32 0');

        // Store the buffer pointer to the parameter
        ir.push('  store i8* ' + buf_ptr + ', i8** ' + param.istack);

        // Initialize buffer to empty string with memset
        ir.push('  call void @memset(i8* ' + buf_ptr + ', i8 0, i64 ' + bufsize + ')');

        // Use scanf to read string from stdin
        if (scanf_var_args) {
          var bufptr = '%' + pre + 'bufptr';
          ir.push('  ' + bufptr + ' = alloca i8*');
          ir.push('  store i8* ' + buf_ptr + ', i8** ' + bufptr);
        ...
        ir.push('  ; STRING READ end');
      } else if (scanf_var_args) {
        ir.push('  ' + call + ' = call i32 (i8*, ...)* @' + scanf_name +
